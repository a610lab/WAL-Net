...
from utils.train_module import train_one_epoch_our, evaluate
...
def main(args):
    ...
    fid_csv = open('../train_aux_save_file/{}_{}_our_{}.csv'.format(args.backbone_name,
                                                                    args.seed,
                                                                    args.lae_method),
                   'w',
                   encoding='utf-8')
    ...
    # 创建模型  构造优化器
    model = model_our.DeepLabV3Plus(model_name=args.backbone_name, lae_method=args.lae_method).to(device)
    ...
    for epoch in range(args.epochs):
        # train
        train_loss, train_acc = train_one_epoch_our(model=model,
                                                    optimizer=optimizer,
        ...
        if best_acc < val_acc:
            ...
            torch.save(model.state_dict(),
                       '../train_aux_save_file/{}_{}_our_{}.pth'.format(args.backbone_name,
                                                                        args.seed,
                                                                        args.lae_method))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lae_method', default='dilated_crop')
    ...
    lae_methods = ['bgrm_crop', 'bgrm', 'rwm', 'crop', 'dilated_crop']
    main(opt)
